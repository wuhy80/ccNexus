...
// ClaudeUsage captures all usage counters returned by Claude, including prompt cache fields.
...
// TotalInputTokens returns the combined prompt tokens, including cache reads/writes.
func (u ClaudeUsage) TotalInputTokens() int {
	return u.InputTokens + u.CacheCreationInputTokens + u.CacheReadInputTokens
}

// ExtractInputTokens sums Claude usage data represented as a generic map (e.g. SSE events).
...
type TokenUsageDetail struct {
	InputTokens              int // Standard input tokens
	CacheCreationInputTokens int // Tokens used for cache creation
...
// ExtractTokenUsageDetail extracts detailed token usage from API response
